...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    ...
    for (int i = 0; i < n; i++) seqList[i] = NULL;
    ...
    for(int i = 0; i < queries_rows; i++){
        ...
        if (type == 1){
            seqList[idx] = realloc(seqList[idx], (sizes[idx]+1)*(sizeof(int)));
            seqList[idx][sizes[idx]] = y;
            sizes[idx]++;
        } else if (type == 2) {
            int value = y % sizes[idx];
            last_answer = seqList[idx][value];
            results[res_count++] = last_answer;
        }
    }
